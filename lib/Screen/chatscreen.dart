import 'package:family_chat/Widget/message_card.dart';
import 'package:family_chat/helper/my_date_util.dart';
import 'package:flutter/cupertino.dart';

import 'package:flutter/material.dart';

import 'package:family_chat/Models/chat_user.dart';
import 'package:cached_network_image/cached_network_image.dart';
import 'package:family_chat/main.dart';

import 'package:family_chat/Api/api.dart';
import 'package:family_chat/Models/message.dart';
import 'package:emoji_picker_flutter/emoji_picker_flutter.dart';
import 'dart:io';
import 'package:image_picker/image_picker.dart';
import 'dart:developer';
import 'view_profile_screen.dart';

class ChatScreen extends StatefulWidget {
  //one user expect
  final ChatUser user;
  const ChatScreen({super.key, required this.user});

  @override
  State<ChatScreen> createState() {
    return _ChatScreenState();
  }
}

class _ChatScreenState extends State<ChatScreen> {
  //for storing all message
  List<Message> _list = [];

  //for handling messages text changes
  final _textController = TextEditingController();
  //for storing value of showing and hiding emoji
  //or checking image uploading or not
  bool showEmoji = false, _isUploading = false;

  @override
  Widget build(BuildContext context) {
    return GestureDetector(
        onTap: FocusScope.of(context).unfocus,
        //click screen hide keyboard
        child: PopScope(
          //if emojis are shown & back button is pressed then hide emojis
          //or else simple close current screen on back button click
          canPop: !showEmoji,
          onPopInvoked: (_) async {
            if (showEmoji) {
              setState(() => showEmoji = !showEmoji);
            } else {
              Navigator.of(context).pop();
            }
          },
          child: Scaffold(
            appBar: AppBar(
              automaticallyImplyLeading: false,
              title: _appBar(),
              backgroundColor: Colors.white,
            ),

            //change backgroung color of screen
            backgroundColor: Colors.blueAccent.shade100,

            body: Column(
              children: [
                Expanded(
                  child: StreamBuilder(
                      //for using getalluser function from api folder
                      stream: Api.getAllMessages(widget.user),
                      builder: (context, snapshot) {
                        //check data load or already load
                        switch (snapshot.connectionState) {
                          //if data is loading
                          case ConnectionState.waiting:
                          case ConnectionState.none:
                            return const SizedBox();

                          //if some or all data load it show it
                          case ConnectionState.active:
                          case ConnectionState.done:
                            final data = snapshot.data?.docs;

                            _list = data
                                    ?.map((e) => Message.fromJson(e.data()))
                                    .toList() ??
                                [];
                            //make list

                            //if no user found in database then
                            if (_list.isNotEmpty) {
                              return ListView.builder(

                                  //show latesh msg
                                  reverse: true,
                                  //if seraching on show search text else showing list chat
                                  itemCount: _list.length,
                                  padding:
                                      EdgeInsets.only(top: mq.height * 0.01),
                                  itemBuilder: (context, index) {
                                    //show current index message
                                    return MessageCard(message: _list[index]);
                                  });
                            } else {
                              return const Center(
                                child: Text(
                                  "Say Hi👋 ",
                                  style: TextStyle(
                                      fontSize: 25,
                                      fontWeight: FontWeight.bold),
                                ),
                              );
                            }
                        }
                      }),
                ),

                //if pic uploading show circlular progress first
                if (_isUploading)
                  const Align(
                      alignment: Alignment.bottomRight,
                      child: Padding(
                        padding:
                            EdgeInsets.symmetric(vertical: 8, horizontal: 20),
                        child: CircularProgressIndicator(
                          strokeWidth: 2,
                        ),
                      )),

                _chatInput(),
                if (showEmoji)
                  EmojiPicker(
                    textEditingController: _textController,
                    config: Config(
                      height: 300,
                      emojiViewConfig: EmojiViewConfig(
                        emojiSizeMax: 32 * (Platform.isIOS ? 1.30 : 1.0),
                      ),
                    ),
                  )
              ],
            ),
          ),
        ));
  }

  //custom app bar
  Widget _appBar() {
    return InkWell(
        onTap: () {
          Navigator.push(
              context,
              MaterialPageRoute(
                  builder: (_) => ViewProfileScreen(
                        user: widget.user,
                      )));
        },
        child: StreamBuilder(
            stream: Api.getUserInfo(widget.user),
            builder: (context, snapshot) {
              final data = snapshot.data?.docs;
              final list =
                  data?.map((e) => ChatUser.fromJson(e.data())).toList() ?? [];

              return Row(
                //firse want back icon button

                children: [
                  //back button
                  //back home screen
                  IconButton(
                      onPressed: () => Navigator.pop(context),
                      icon: const Icon(
                        Icons.arrow_back,
                        color: Colors.black54,
                      )),

                  ClipRRect(
                    borderRadius: BorderRadius.circular(mq.height * .03),
                    //user profile pic
                    child: CachedNetworkImage(
                      width: mq.height * .05,
                      height: mq.height * .05,
                      fit: BoxFit.cover,
                      imageUrl:
                          list.isNotEmpty ? list[0].image : widget.user.image,
                      placeholder: (context, url) =>
                          const CircularProgressIndicator(),
                      errorWidget: (context, url, error) =>
                          const Icon(Icons.error),
                    ),
                  ),

                  const SizedBox(
                    width: 10,
                  ),
                  Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      //user name
                      Text(
                        list.isNotEmpty ? list[0].name : widget.user.name,
                        style: const TextStyle(
                          color: Colors.black87,
                          fontWeight: FontWeight.bold,
                          fontSize: 12,
                        ),
                      ),

                      const SizedBox(
                        height: 1,
                      ),
                      //last seen user
                      Text(
                        list.isNotEmpty
                            ? list[0].isOnline
                                ? "Online"
                                : MyDateUtil.getLastActiveTime(
                                    context: context,
                                    lastActive: list[0].lastActive)
                            : MyDateUtil.getLastActiveTime(
                                context: context,
                                lastActive: widget.user.lastActive),
                        style: const TextStyle(
                          color: Colors.black54,
                          fontWeight: FontWeight.bold,
                          fontSize: 12,
                        ),
                      ),
                    ],
                  ),
                ],
              );
            }));
  }

  Widget _chatInput() {
    return Padding(
      padding: EdgeInsets.symmetric(
          vertical: mq.height * .01, horizontal: mq.width * .025),
      child: Row(
        children: [
          //input field and button
          Expanded(
            child: Card(
              shape: RoundedRectangleBorder(
                  borderRadius: BorderRadius.circular(15)),
              child: Row(
                children: [
                  IconButton(
                      onPressed: () {
                        //hide emozi bar when click screen

                        setState(() {
                          FocusScope.of(context).unfocus();
                          //if true then false if false then true
                          showEmoji = !showEmoji;
                        });
                      },
                      icon: const Icon(
                        Icons.emoji_emotions,
                        color: Colors.blueAccent,
                        size: 25,
                      )),
                  Expanded(
                    //adjust automatically when user write long message
                    child: TextField(
                      controller: _textController,
                      keyboardType: TextInputType.multiline,
                      maxLines: null,
                      onTap: () {
                        //when click keyword only show keybaord no emoji
                        if (showEmoji) {
                          setState(() {
                            //if true then false if false then true
                            showEmoji = !showEmoji;
                          });
                        }
                      },
                      decoration: const InputDecoration(
                        hintText: "Type Something",
                        hintStyle: TextStyle(color: Colors.black54),
                        border: InputBorder.none,
                      ),
                    ),
                  ),
                  //take multiple image or image from gallery icon
                  IconButton(
                      onPressed: () async {
                        final ImagePicker picker = ImagePicker();

                        //pick a multiple images
                        final List<XFile> images =
                            await picker.pickMultiImage(imageQuality: 70);

                        //uploading and sending image by one by one
                        for (var i in images) {
                          //if pic upload set uploading true
                          log("Image Path : ${i.path}");
                          setState(() => _isUploading = true);
                          await Api.sendChatImage(widget.user, File(i.path));
                          setState(() => _isUploading = false);
                        }
                      },
                      icon: const Icon(
                        Icons.image,
                        color: Colors.blueAccent,
                        size: 25,
                      )),

                  //take image from camera button
                  IconButton(
                      onPressed: () async {
                        final ImagePicker picker = ImagePicker();

                        //pick a image
                        final XFile? image = await picker.pickImage(
                            source: ImageSource.camera, imageQuality: 70);
                        if (image != null) {
                          log('Image Path: ${image.path}');
                          //if pic upload set uploading true
                          setState(() => _isUploading = true);

                          await Api.sendChatImage(
                              widget.user, File(image.path));
                          setState(() => _isUploading = false);
                        }
                      },
                      icon: const Icon(
                        Icons.camera_alt,
                        color: Colors.blueAccent,
                        size: 25,
                      )),
                  SizedBox(
                    width: mq.width * .02,
                  ),
                ],
              ),
            ),
          ),
          //send message button
          MaterialButton(
            onPressed: () {
              if (_textController.text.isNotEmpty) {
                if (_list.isEmpty) {
                  //on first message (add user to my_user collection of chat user)
                  Api.sendFirstMessage(
                      widget.user, _textController.text, Type.text);
                } else {
                  //simply send message
                  Api.sendMessage(widget.user, _textController.text, Type.text);
                }
                _textController.text = '';
              }
            },
            minWidth: 0,
            padding:
                const EdgeInsets.only(top: 10, bottom: 10, right: 5, left: 10),
            shape: const CircleBorder(),
            color: Colors.green,
            child: const Icon(
              Icons.send,
              color: Colors.white,
              size: 28,
            ),
          )
        ],
      ),
    );
  }
}
